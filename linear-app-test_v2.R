...
plot_demand <- scenario_demand %>%
  ...
  summarise(af_monthly = sum(af_monthly, na.rm = TRUE),
    ...
    select(d_scenario, s_scenario, plot_date,
           fill_color, af_daily, cfs, plot_group)

ggplot(data = plot_demand,
       aes(x = plot_date,
           y = cfs)) +
  geom_area(data = subset(plot_demand, fill_color = "Post-14"),
            position = "stack",
            aes(fill = fill_color)) + 
...
## Add boundary points to facilitate correct stacking.

plot_demand <- bind_rows(old = plot_demand,
                         new = mutate(plot_demand, 
...
vsd_plot_data <- bind_rows(filter(demand[[huc8_selected]], 
                              d_scenario %in% d_scene_selected) %>% 
                         mutate(fill_color = if_else(priority == "Statement Demand",
                                                     "Statement Demand",
                                                     if_else(priority == "Statement Demand",
...
# Demand
g <- g + geom_area(data = subset(plot_demand, plot_group == "demand"),
                   position = "stack",
                   aes(fill = fill_color))

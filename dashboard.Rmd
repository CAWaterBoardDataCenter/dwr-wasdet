...
## Define color palettes and order. ----

# Water right types.
# wr_type_list <- ordered(
#   c("Appropriative",
#     "Federal Claims",
#     "Federal Stockponds",
...
wr_type_pal <-colorFactor(c(wes_palette("Darjeeling1"), 
                          ...
                          pods$wr_type)
# names(wr_type_pal) <- wr_type_list

# Water availability demand stacking order.
...
renderPlot({
 ...
 scale_fill_manual(values = wa_demand_pal)
...
pod_points <- reactive({
  pods %>%
    filter(wr_id %in% ws_wr_info()$wr_id)
})
